...
const Header = () => {
  ...
  const menuItems = {
    ...
    forParents: [
      { linkTo: '/sabak-torcho', title: 'Сабак торчо' },
      { linkTo: '/', title: 'Күн тартип' },
      { linkTo: '/', title: 'Сабак жүгүртмөсү' },
      { linkTo: '/', title: 'Табель' },
  ...
  const toggleMenu = (menuName) => {
    setOpenMenu((prevMenu) => ({ ...prevMenu, [menuName]: !prevMenu[menuName] }));
  };
              ...
              {openMenu.forKids &&
                <ul className={`dropdown-menu display`}>
                  {menuItems.forKids.map(({ linkTo, title }, index) => (
                    <li key={index}>
                      <Link to={linkTo} onClick={() => handleMouseLeave('forKids')}>{title}</Link>
                    </li>
                  ))}
                </ul>
              }
              ...
              {openMenu.forParents &&
                <ul className={`dropdown-menu2 display`}>
                  {menuItems.forParents.map(({ linkTo, title }, index) => (
                    <li key={index}>
                      <Link to={linkTo} onClick={() => handleMouseLeave('forParents')}>{title}</Link>
                    </li>
                  ))}
                </ul>
              }
              ...
              {openMenu.gallery &&
                <ul className={`dropdown-menu3 display`}>
                  {menuItems.gallery.map(({ linkTo, title }, index) => (
                    <li key={index}>
                      <Link to={linkTo} onClick={() => handleMouseLeave('gallery')}>{title}</Link>
                    </li>
                  ))}
                </ul>
              }
